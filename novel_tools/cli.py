...
def _inspect_book(root: Path, book_id: str, stdout: TextIO) -> int:
    ...
    print(f"Chinese chapters: {_count_files(paths.cn_chapters_dir, 'chapter_????_cn.txt')}", file=stdout)
    print(f"Vietnamese chapters: {_count_files(paths.vi_chapters_dir, 'chapter_????.txt')}", file=stdout)
    print(f"Progress entries: {_count_files(paths.progress_dir, 'chapter_????.json')}", file=stdout)
